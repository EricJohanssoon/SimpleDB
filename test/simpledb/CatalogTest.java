...
public class CatalogTest extends SimpleDbTestBase {
    ...
    @Test public void handleDuplicateIds() throws Exception {
        ...
        String newName2 = SystemTestUtil.getUUID();
        int id3 = r.nextInt();
        DbFile f2 = new SkeletonFile(id3, Utility.getTupleDesc(2));
        Database.getCatalog().addTable(f2, newName2);

        String newName3 = SystemTestUtil.getUUID();
        int id4 = r.nextInt();
        DbFile f3 = new SkeletonFile(id4, Utility.getTupleDesc(2));
        Database.getCatalog().addTable(f3, newName3);

        String newName4 = SystemTestUtil.getUUID();
        int id5 = r.nextInt();
        DbFile f4 = new SkeletonFile(id5, Utility.getTupleDesc(2));
        Database.getCatalog().addTable(f4, newName4);

        String newName5 = SystemTestUtil.getUUID();
        int id6 = r.nextInt();
        DbFile f5 = new SkeletonFile(id6, Utility.getTupleDesc(2));
        Database.getCatalog().addTable(f5, newName5);


    	DbFile f = new SkeletonFile(id2, Utility.getTupleDesc(2));
    	Database.getCatalog().addTable(f, newName);
        String name2 = Database.getCatalog().getTableName(id2);
